...
- Setup de Prisma: configura `DATABASE_URL` con el PostgreSQL del servidor casero. La base ya está migrada: no ejecutes `prisma migrate` ni recrees datos durante un deploy normal.
- **La base ACTIVA es la que diga `DATABASE_URL` en `.env`**: es el **PostgreSQL local del servidor casero**, y `schema.prisma` declara `provider = "postgresql"`. Scraping y curación corren contra datos vivos: **todo script que ejecutes toca producción**. Los `.db` de `prisma/` son respaldos históricos de solo lectura; nunca los consultes ni asumas SQLite. Los snapshots son dumps JSON vía `pg-snapshot.ts` (`snapshot-save.ps1`/`snapshot-restore.ps1`).
